...
BasicMM <- function(
  ...
  # Get Data
  myList <- list()
  myList <- lapply(target, function(x) {quantmod::getSymbols(x, from = as.Date("1990-01-04"), auto.assign = FALSE)})
  names(myList) <- target
  target <- myList[[1]]
...
AdvBuySellAlgorithm <- function(
  ...
  # Dygraph
  p2 <- dygraphs::dygraph(
    buysellTable$TargetClosingPrice,
    ...
    navbarPage(
      ...
      tabPanel("Navbar 1: TIMING STOCK MARKET",
               sidebarPanel(
                 width = 12,
                 fluidRow(
                   column( 4, textInput("ticker", "Target Stock:", "AAPL") ),
                   column( 8, sliderInput("coef", "Buy/Sell Coefficients", min = -30, max = +30, value = c(-10, 10), step = 0.1) )
                 ),
                 fluidRow(
               ...
               mainPanel(
                 ...
                 tabsetPanel(
                   tabPanel("Basics",
                            ...
                            dygraphOutput("basicMMstats_Plot", width = "100%", height = "300px"),
                   ...
                   tabPanel("Yin's Timer",
                            ...
                            dygraphOutput("timing_FinalPlot", width = "100%", height = "300px"),
